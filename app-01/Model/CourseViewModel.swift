...
class CourseViewModel: ObservableObject {
    ...
    func isCourseOnDate(course: Course, date: Date) -> Bool {
        let calendar = Calendar.current
        let weekday = calendar.component(.weekday, from: date)
        return course.daysOfWeek.contains(weekday)
    }
    
    func coursesForDate(_ date: Date) -> [Course] {
        return courses.filter { course in
            isCourseOnDate(course: course, date: date)
        }
    }
}
